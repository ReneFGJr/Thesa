...
class skoses extends CI_model {
    ...
    */
    ...
    function le_c($id, $th = '') {
        ...
        LEFT JOIN th_concept_term ON ct_concept = id_c
        LEFT JOIN rdf_literal ON id_rl = ct_term
        LEFT JOIN th_thesaurus ON id_pa = ct_th
    ...
    function export_to_thesa2($id)
        {
            ...
            $thesa = array();
            $thesa['name'] = $rlt[0]['pa_name'];
            $thesa['description'] = $rlt[0]['pa_description'];
            $thesa['introdution'] = $rlt[0]['pa_introdution'];
            $thesa['methodology'] = $rlt[0]['pa_methodology'];
            $thesa['icone'] = $rlt[0]['pa_icone'];
            $thesa['type'] = $rlt[0]['pa_type'];
            $thesa['language'] = $rlt[0]['pa_language'];
            $thesa['multi_language'] = 1;

            $data['thesa'] = $thesa;
            $data['terms'] = $this->export_to_thesa2_concept($id);

            echo json_encode($data);
            exit;
        }
    function export_to_thesa2_concept($id)
        {
            $sql = "select * from th_concept where c_th = ".$id;
            $rlt = $this->db->query($sql);
            $rlt = $rlt->result_array();
            ...
            for ($r=0;$r < count($rlt);$r++)
                {
                    ...
                    $sql = "select * from th_concept_term
                            left join rdf_literal on id_rl = ct_term
                            left join rdf_resource on id_rs = ct_propriety
                            where ct_concept = ".$idc;

                    $drlt = $this->db->query($sql);
                    $drlt = $drlt->result_array();

                    for ($y=0;$y < count($drlt);$y++)
                        {
                            $dline = $drlt[$y];
                            $data[$idc][$dline['rl_value']] =
                                array(
                                    'proprety'=>$dline['rs_propriety'],
                                    'lang'=>$dline['rl_lang']
                                );
                        }
                }
    ...
    */
    ...
    function assign_as_narrower($c1, $c2, $th, $tm) {
        ...
        if (count($rlt) == 0)
        {
            ...
            $idct = $rlt[0]['id_ct'];
            $sql = "update th_concept_term set ct_concept = $c1 where id_ct = $idct";
        }
        $rlt = $this -> db -> query($sql);
        exit;
    }
    ...
    function glossario_html($th, $lk) {
        ...
        t1.ct_concept as c, note.rl_value as note,
        note2.rs_propriety as prop_note, pre2.prefix_ref as prop_pre,
        UPPER(substr(r1.rl_value,1,1)) as ltr, note.rl_lang as lang
        FROM th_concept_term as t1
        INNER JOIN rdf_literal AS r1 ON id_rl = t1.ct_term
        INNER JOIN rdf_resource as note0 ON id_rs = t1.ct_propriety
        LEFT JOIN th_concept_term as t2 on t1.ct_concept = t2.ct_concept and t2.ct_propriety = 25
        ...
        for ($r = 0; $r < count($rlt); $r++) {
            ...
            if ($rem == 0) {
                ...
                //print_r($line);
                //exit;
                if ((strlen($nota) > 0) and ($xnota != $nota)) {
                        $xnota = $nota;
                    $nota .= '<sup>('.$line['lang'].')</sup>';
...
function glossario_alfabetico_html($th, $lk) {
    ...
    $sql = "select
    r1.rl_value as rl_value, r1.rl_lang as rl_lang,
    ...
    for ($r = 0; $r < count($rlt); $r++) {
        ...
        //$sx .= '<br>';
        $sx .= '<br>';
        $sx .= '<b>' . $line['rl_value'] . ' <sup>('.$line['rl_lang'].')</sup></b>';
...
function ficha_terminologica_html($th, $lk) {
    ...
    for ($r = 0; $r < count($rlt); $r++) {
        ...
        for ($z = 0; $z < count($d['notes']); $z++) {
            $xnote .= '<br>' . $d['notes'][$z]['rl_value'];
                //echo '<pre>';
                //print_r($d['notes'][$z]);
                //exit;
                $ynote = $d['notes'][$z]['rl_value'];
                $note = $d['notes'][$z];
                if ($ynote != $xnote)
                {
                    $xnote = $ynote;
...
//        $lang = 'pt';
    ...
    function skos_xml($th = 0)
    {
        ...
        $sx = '<?xml version="1.0" encoding="UTF-8"?>'.cr();
        $sx .= '<RDF xmlns="http://www.w3.org/2004/02/skos/core#" '.cr();;
        ...
        for ($r=0;$r < count($rlt);$r++)
        {
            ...
            if ($xcp != $cp)
            {
                if ($cp > 0)
                {
                    ...
                    {
                        $sx .= '<broader resource="'.base_url(PATH.'c/'.trim($line['ct_concept'])).'"/>'.cr();
                    }

                    /* mostra notas de escopo */
                    $sql = "select * from rdf_literal_note ";
                    $sql .= "inner join rdf_resource ON rl_type = id_rs ";
                    $sql .= "where rl_c = ".$cp;
                    $rrr = $this->db->query($sql);
                    $rrr = $rrr->result_array();
                    for ($y=0;$y < count($rrr);$y++)
                    {
                        $pclass = $rrr[$y]['rs_propriety'];
                        $note = troca(trim($rrr[$y]['rl_value']),'&','&amp;');
                        $note = troca($note,'<','&lt;');
                        $note = troca($note,'>','&gt;');
                        $sx .= '<scopeNote type="'.$pclass.'" lang="'.$rrr[$y]['rl_lang'].'">'.$note. '</scopeNote>'.cr();
                    }
                    $sx .= '</Concept>'.cr();
                }
                $sx .= '<Concept about="'."thesa:".$xcp.'#">'.cr();
            }
            ...
            switch($prop)
            {
                case 'prefLabel':
                    ...
                    {
                    $sx .= '<prefLabel lang="'.$this->language_skos($line['rl_lang']).'" >'.troca(trim($line['rl_value']),'&','&amp;').'</prefLabel>'.cr();
                    $pref = True;
                    }
                break;

                case 'hiddenLabel':
                    $sx .= '<hiddenLabel lang="'.$this->language_skos($line['rl_lang']).'" >'.troca(trim($line['rl_value']),'&','&amp;').'</hiddenLabel>'.cr();
                    $pref = True;
                break;
                ...
                {
                    case 'FE':
                        $sx .= '<altLabel lang="'.$this->language_skos($line['rl_lang']).'" >'.troca(trim($line['rl_value']),'&','&amp;').'</altLabel>'.cr();
                        $pref = True;
                    break;
                    case 'TG':
                        $sx .= '<narrower resource="'."thesa:".trim($line['ct_concept_2']).'"/>'.cr();
                        $pref = True;
                    break;
                    case 'TR':
                        $sx .= '<related resource="'."thesa:".trim($line['ct_concept_2']).'"/>'.cr();
                        $pref = True;
                    break;
                    default:
        ...
        if ($tipo == 'INS') {
            ...
            switch ($type) {
                case 'xml' :
                    ...
                    $sr .= '<RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:si="https://www.w3schools.com/rdf/">' . cr();
