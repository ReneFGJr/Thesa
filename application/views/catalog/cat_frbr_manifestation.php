<h2><?php echo msg('Manifestation'); ?></h2>
<?php
$group = lowercase('WORKMANI');
...
for ($r=0;$r < count($qrlt);$r++)
	{
		$line = $qrlt[$r];
		$link = ' <a href="#" onclick="newwin(\'' . base_url('index.php/catalog/input/'.$group.'/' . $it_id_i . '/' . $line['id_rs'].'/'.$line['rs_class'].'/'.checkpost_link($it_id_i.$line['id_rs'])) . '\',500,500);">';
		...
		echo '<ul>';
		for ($y=0;$y < count($work_manifestation);$y++)
			{
				$xline = $work_manifestation[$y];
...
<h4><?php echo msg('Place of publication');?></h4>

<h4><?php echo msg('Form of carrier');?></h4>
