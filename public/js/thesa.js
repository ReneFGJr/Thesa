...
function thesa_reference_set($id, $ref, item) {
  ...
  if ($(item).is(":checked")) {
    var url =
      $path +
      "/admin/ajax_concept_reference?id=" +
      $id +
      "&ref=" +
      $ref +
      "&set=1";
...
function form_thesa_refecence_save($id, $prop, $term) {
  ...
  var url =
    $path +
    "/admin/ajax_form_reference_save?id=" +
    $id +
    "&prop=" +
    $prop +
    "&term=" +
...
function load_content(value,PATH='') {
